...
from scratch.prefetch_option_data import prefetch_all

def run_buy_and_hold(stock_df: pd.DataFrame, starting_capital: float) -> pd.DataFrame:
    """
    Simulates Buy & Hold benchmark for the configured asset.
...
def main():
    ...
    # 2. Run Options Data Pre-fetcher as an automatic pre-check
    prefetch_all(config_path="config.yaml", creds_path="creds.txt", max_workers=3)
    
    # Enforce offline backtesting using pre-fetched cache
    loader = DataLoader(creds_path="creds.txt")
    loader.is_mock = True
    
    print(f"Loading stock daily database...", flush=True)
    ...
    if 'metrics_dict' not in optimal_run:
        ...
        engine = BacktestEngine(run_config, loader)
        opt_history = engine.run_backtest().to_pandas()
    ...
    # Benchmark A: Buy & Hold underlying asset
    ...
    otm_history = otm_engine.run_backtest().to_pandas()
    ...
    plt.plot(bh_history['date'], bh_history['portfolio_value'], label=f'Buy & Hold {config["asset"]}', color='#ef4444', linewidth=1.5, alpha=0.8)
    ...
    print("==============================================================", flush=True)
    gap_header = f"{config['asset']} Overnight Gap"
    print(f"{'Date':<12} | {gap_header:<20} | {'Portfolio Daily Return':<22} | {'Portfolio Drawdown':<20}", flush=True)
    print("-" * 83, flush=True)
